...
pygame.display.set_caption('F1 Racing Game By AJ Games')
...
# Scale car images
carLeft = pygame.transform.scale(carLeft, (int(carLeft.get_width() * 0.2), int(carLeft.get_height() * 0.2)))
carRight = pygame.transform.scale(carRight, (int(carRight.get_width() * 0.2), int(carRight.get_height() * 0.2)))
carImg = pygame.transform.scale(carImg, (int(carImg.get_width() * 0.45), int(carImg.get_height() * 0.45)))
...
def high_score_update(dodged):
    with open(os.getcwd() + '/high_score.txt', 'w') as hs:
        hs.write(str(dodged))
...
def title_msg(shift_x, shift_y, color):
    ...
    lines = ["F1 Racing Games", "By AJ Games"]
    y_offset = display_height / 3 - shift_y
    for line in lines:
        TextSurf, TextRect = text_objects(line, largeText, color)
        TextRect.center = (display_width / 2 - shift_x, y_offset)
        gameDisplay.blit(TextSurf, TextRect)
        y_offset += largeText.get_height()
    pygame.display.update()
    time.sleep(0.15)
...
def game_loop():
    ...
    with open(os.getcwd() + '/high_score.txt', 'r') as high_score_file:
